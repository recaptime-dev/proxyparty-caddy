...
app.use("/admin/*", bearerAuth({
  verifyToken: async (token, c) => {
    ...
    if (token === adminToken) {
      formatLogger(`Admin auth token verification succeeded for request at ${c.req.url}`, c.get('requestId'));
      return true;
    }
    formatLogger(`Admin auth token verification failed for request at ${c.req.url} (was ${token})`, c.get('requestId'));
    return false;
  }
}))
...
app.openAPIRegistry.registerComponent('securitySchemes', 'AdminApiToken', {
  type: 'http',
  scheme: 'bearer',
  description: "Admin API token for admin endpoints. Managed via `PROXYPARTY_ADMIN_TOKEN` variable on the backend.",
})
...
app.doc31("/openapi.json", (c) => ({
  ...
  info: {
    ...
    version: "0.1.0",
    description: "API service for Proxyparty internals, including On-Demand TLS domain checks as used by Caddy.",
    contact: {
  ...
  tags: [
    ...
    },
    {
      ...
      description: "API endpoints used by Caddy that are also available to use publicly."
    }
  ]
}))
